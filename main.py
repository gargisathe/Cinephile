...
def genre_recommender(genre):
    ...
    sorted(list(enumerate(genre_similarity[0])), reverse=True, key = lambda x:x[1])[1:6]
    index = main_df[main_df['genres'] == genre].index[0]
    distances = sorted(list(enumerate(genre_similarity[index])),reverse=True,key = lambda x: x[1])
