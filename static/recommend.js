...
//     var my_api_key = 'a6270c31daee789ef6c54ceedce0a34c';
//     var title = $('.movie').val();
//     if (title=="") {
...
$(function () {
  // Button will be disabled until we type anything inside the input field
  const source = document.getElementById('autoocomplete');
  const inputHandler = function (e) {
    if (e.target.value == "") {
      $('.movie-button').attr('disabled', true);
    }
...
function genre_movie_recs(api_key) {
  $.ajax({
    type: 'POST',
    url: "/similar_genre",
    success: function (recs) {
      if (recs == "Sorry! The genre you requested is not in our database. Please check the spelling or try with some other genres") {
        $('.fail').css('display', 'block');
        $('.results').css('display', 'none');
        $("#loader").delay(500).fadeOut();
      }
...
function show_details_genre(arr){
  arr_poster = get_movie_posters(arr, my_api_key);
  details = {
    'rec_movies': JSON.stringify(arr),
    'rec_posters': JSON.stringify(arr_poster)
